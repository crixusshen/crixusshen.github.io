(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["docs/spec/introduce.en-US.md"],{

/***/ "./node_modules/bisheng/lib/loaders/source-loader.js!./docs/spec/introduce.en-US.md":
/*!******************************************************************************************!*\
  !*** ./node_modules/bisheng/lib/loaders/source-loader.js!./docs/spec/introduce.en-US.md ***!
  \******************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = {
  "content": [
    "article",
    {

    },
    [
      "h2",
      "Guidelines and Resources"
    ],
    [
      "p",
      "We supply a series of design principles, practical patterns and high quality design resources (Sketch and Axure), to help people create their product prototypes beautifully and efficiently."
    ],
    [
      "ul",
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "/docs/spec/proximity"
            },
            "Principles"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "/docs/pattern/navigation"
            },
            "Patterns"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "/docs/resource/download"
            },
            "Resource Download"
          ]
        ]
      ]
    ],
    [
      "h2",
      "Front-end Implementation"
    ],
    [
      "p",
      [
        "a",
        {
          "title": null,
          "href": "http://facebook.github.io/react/"
        },
        "React"
      ],
      " is used to encapsulate a library of components which embody our design language. We welcome the community to implement ",
      [
        "a",
        {
          "title": null,
          "href": "https://ant.design/docs/spec/introduce"
        },
        "our design system"
      ],
      " in other front-end frameworks of their choice."
    ],
    function bishengPluginReactPreviewer() {
  var React = __webpack_require__(/*! react */ "./node_modules/react/index.js");

  var ReactDOM = __webpack_require__(/*! react-dom */ "./node_modules/react-dom/index.js");

  var _antd = __webpack_require__(/*! antd */ "./index.js");

  var LinkIcon = function LinkIcon() {
    return React.createElement(_antd.Icon, {
      type: "export",
      className: "outside-link-icon"
    });
  };

  var LinksList = function LinksList() {
    return React.createElement("ul", null, React.createElement("li", null, React.createElement("a", {
      href: "/docs/react/introduce",
      target: "_blank"
    }, "Ant Design of React"), "(official implementation)"), React.createElement("li", null, React.createElement("a", {
      href: "http://ng.ant.design",
      target: "_blank"
    }, "NG-ZORRO - Ant Design of Angular", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "http://ng.mobile.ant.design",
      target: "_blank"
    }, "NG-ZORRO-MOBILE - Ant Design Mobile of Angular", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "http://vue.ant.design",
      target: "_blank"
    }, "Ant Design of Vue", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "https://ecomfe.github.io/santd",
      target: "_blank"
    }, "San UI Toolkit for Ant Design", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "https://github.com/FE-Driver/vue-beauty",
      target: "_blank"
    }, "vue-beauty (vue)", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "https://github.com/priornix/antizer",
      target: "_blank"
    }, "antizer (ClojureScript)", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "https://github.com/idcos/antd-ember",
      target: "_blank"
    }, "antd-ember", React.createElement(LinkIcon, null))), React.createElement("li", null, React.createElement("a", {
      href: "https://github.com/zzuu666/antue",
      target: "_blank"
    }, "antue (vue)", React.createElement(LinkIcon, null))));
  };

  return React.createElement(LinksList, null);
},
    [
      "h2",
      "Who's using Ant Design"
    ],
    [
      "ul",
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.antgroup.com/index.htm?locale=en_US"
            },
            "Ant Financial"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.alibaba.com/"
            },
            "Alibaba"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.tencent.com"
            },
            "Tencent"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.baidu.com"
            },
            "Baidu"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.koubei.com/"
            },
            "Koubei"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.meituan.com"
            },
            "Meituan"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "http://www.xiaojukeji.com/"
            },
            "Didi"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "https://www.ele.me/"
            },
            "Eleme"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          [
            "a",
            {
              "title": null,
              "href": "https://github.com/ant-design/ant-design/issues/477"
            },
            "Other Users"
          ]
        ]
      ]
    ],
    [
      "blockquote",
      [
        "p",
        "If your company or products use Ant Design, and you'd like to be added to this growing list, click ",
        [
          "a",
          {
            "title": null,
            "href": "https://github.com/ant-design/ant-design/issues/477"
          },
          "here"
        ],
        " to leave us a message."
      ]
    ],
    [
      "h2",
      "Words From Community"
    ],
    [
      "ul",
      [
        "li",
        [
          "p",
          "Hacknews: ",
          [
            "a",
            {
              "title": null,
              "href": "https://news.ycombinator.com/item?id=13053137"
            },
            "Show HN: Antd – A set of high-quality React components"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          "Alligator: ",
          [
            "a",
            {
              "title": null,
              "href": "https://alligator.io/react/beautiful-uis-ant-design/"
            },
            "Crafting Beautiful UIs in React Using Ant Design"
          ]
        ]
      ],
      [
        "li",
        [
          "p",
          "Hackernoon: ",
          [
            "a",
            {
              "title": null,
              "href": "https://hackernoon.com/interesting-javascript-libraries-born-in-china-d50d1bb81355"
            },
            "Interesting JavaScript Libraries born in China"
          ]
        ]
      ]
    ],
    [
      "h2",
      "How to Contribute"
    ],
    [
      "p",
      "Contributions to Ant Design on Github are welcomed! Whether you have questions, concerns, or suggestions for improving Ant Design - please don't hesitate to reach out to us ",
      [
        "a",
        {
          "title": null,
          "href": "https://github.com/ant-design/ant-design/issues"
        },
        "here"
      ],
      "."
    ]
  ],
  "meta": {
    "category": "Ant Design",
    "order": 0,
    "title": "Introduction",
    "filename": "docs/spec/introduce.en-US.md"
  },
  "description": [
    "section",
    [
      "div",
      {
        "style": "text-align: center; margin: 40px 0px;"
      },
      [
        "img",
        {
          "width": "600",
          "src": "https://gw.alipayobjects.com/zos/rmsportal/lcamFWetlMgLkLmDUgmZ.png"
        }
      ]
    ],
    [
      "p",
      "In the process of internal desktop applications development, many different design specs and implementations would be involved, which might cause designers and developers difficulties and duplication and reduce the efficiency of development. After massive project practice and summaries, Ant Design, a design language for background applications, is refined by Ant UED Team, which aims to uniform the user interface specs for internal background projects, lower the unnecessary cost of design differences and implementation and liberate the resources of design and front-end development."
    ],
    [
      "p",
      "Ant Design which is specially created for internal desktop applications, is committed to improving the experience of users and product designers. User interface designers and user experience designers, collectively, are considered as product designers, and the boundaries of product managers, interaction designers, visual designers, front-end developers and develop engineers are blurred. Taking advantage of unitary specifications, Ant Design makes design and prototype more simple and accessible for all project members, which comprehensively promotes experience and development efficiency of background applications and products."
    ]
  ],
  "toc": [
    "ul",
    [
      "li",
      [
        "a",
        {
          "className": "bisheng-toc-h2",
          "href": "#Guidelines-and-Resources",
          "title": "Guidelines and Resources"
        },
        "Guidelines and Resources"
      ]
    ],
    [
      "li",
      [
        "a",
        {
          "className": "bisheng-toc-h2",
          "href": "#Front-end-Implementation",
          "title": "Front-end Implementation"
        },
        "Front-end Implementation"
      ]
    ],
    [
      "li",
      [
        "a",
        {
          "className": "bisheng-toc-h2",
          "href": "#Who's-using-Ant-Design",
          "title": "Who's using Ant Design"
        },
        "Who's using Ant Design"
      ]
    ],
    [
      "li",
      [
        "a",
        {
          "className": "bisheng-toc-h2",
          "href": "#Words-From-Community",
          "title": "Words From Community"
        },
        "Words From Community"
      ]
    ],
    [
      "li",
      [
        "a",
        {
          "className": "bisheng-toc-h2",
          "href": "#How-to-Contribute",
          "title": "How to Contribute"
        },
        "How to Contribute"
      ]
    ]
  ]
};

/***/ })

}]);
//# sourceMappingURL=introduce.en-US.md.js.map